...
function SignupScreen() {
  ...
  const register = (e) => {
    ...
    auth
      ...
      .then((authUser) => {
        //then once it is created it will give me back the user info
        console.log(authUser);
      })
      .catch((error) => {
        //if at any point there is an error it will catch the error and alert that on the screen
        alert(error.message);
      });
  };
  ...
  return (
    <div className="signupScreen">
      <form>
        ...
        {/*here attaching the ref so refernce for the email field is emailRef */}
        <input ref={passwordRef} placeholder="Password" type="password" />{" "}
        {/*here attaching the ref so refernce for the passwrd field is passwrdRef */}
        <button type="submit" onClick={signIn}>
          Sign In
        </button>{" "}
        {/*it will trigger the signin function  */}
        <h4>
          ...
          {/*it will trigger the register function  */}
        </h4>
      </form>
    </div>
  );
