...
function App() {
  ...
  useEffect(() => {
    //useeffect is basically listens to the users loggedin state
    const unsubscribe = auth.onAuthStateChanged((userAuth) => {
      //this is something called a listener coz it listens to any authenticated state change and the advntage off firebase is whenever u go ahead even if u are loggedin n u refresh it will store it to your local memory in ur browser in a cookie and it knows that u are logedin from before
      if (userAuth) {
        //if the user exist              //onAuthStateChanged gives us something called a userAuth call back
        //loggedin
        // console.log(userAuth);
        dispatch(
          //we are dispatching the login event
          login({
            //it will push the user into the store
    ...
    <div className="app">
      <Router>
        {!user ? ( //if there is no user i want to render the loginScreen
          ...
          <Switch>
            {" "}
            {/* A <Switch> looks through its children <Route>s an renders the first one that matches the current URL. */}
            <Route path="/profile">
              <ProfileScreen />
            </Route>
            <Route path="/profiles">
